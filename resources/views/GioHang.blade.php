    ...
    <div class="row">
        ...
        <div class="cart-items-container col-md-9 pe-md-4">
            ...
            @foreach ($cartItems as $item)
                <div class="cart-item d-flex mt-3 align-items-center" id="item{{ $item->id }}">
                    <input type="checkbox" class="cart-item-checkbox mx-md-3 mx-2">
                    <img src="{{ asset($item->book->images->first()->url) }}" alt="Product Image" class="cart-item-image">
                    <div class="cart-item-name">{{ $item->book->name }}</div>
                    <div class="cart-item-quantity d-flex mx-md-3 mx-2 ">
                        <button onclick="changeQuantity('decrease', {{ $item->id }})">-</button>
                        <input type="text" value="{{ $item->quantity }}" id="quantity{{ $item->id }}" onchange="updateItemTotal({{ $item->id }})">
                    ...
                    <div class="cart-item-price d-flex flex-md-row flex-column">
                        ...
                        <span class="cart-item-total-price ms-md-3" id="total-price{{ $item->id }}" data-unit-price="{{ $item->book->unit_price }}">
